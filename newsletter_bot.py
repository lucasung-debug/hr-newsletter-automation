...
# ============================================================
FIXED_KEYWORDS = {
    "MACRO": ["오뚜기", "식품 원자재 가격", "라면 시장"],
    "HR": ["주52시간제", "교대근무", "임금피크제"],
}

FALLBACK_KEYWORDS = {
    "MACRO": ["식품 수출 관세", "환율 수출", "식품안전 규제", "식품제조업 산업안전"],
    "HR": ["고용노동부 제조업", "윤리경영 ESG", "제조업 인사노무", "임금체계 개편"],
}
...
[조건]
- macro: 거시경제·식품산업·원자재·환율·수출입 관련 검색어 4개
- hr: 인사노무·노동법·교대근무·윤리경영·산업안전 관련 검색어 4개
...
[규칙]
1. part1 (거시경제+식품산업): M- 뉴스 중 경영 임팩트 상위 3개 선정
...
# 6. AI 심층 리포트 (PART 3)
# ============================================================
def generate_deep_report(api_key, hr_news_ctx):
    """BCG 수준의 HR 심층 분석 아티클 생성."""
    prompt = f"""당신은 글로벌 경영 컨설팅 펌의 시니어 HR 컨설턴트입니다.

[회사 컨텍스트]
{COMPANY_CONTEXT}

아래 HR/인사노무 뉴스 데이터 중 가장 시사점이 큰 주제 1개를 선정하고,
해당 주제에 대해 심층 분석 아티클을 작성하세요.

[작성 규칙]
- 구조: 도입(이슈 제기, 2~3문장) → 본론(사례/데이터 기반 분석 2~3단락) → 시사점(오뚜기라면 HR 관점)
- 분량: 800~1200자
- 톤: 전문 컨설팅 리포트 (불필요한 수식어 배제, 데이터·논리 중심)
- 리더십, 조직문화, 인사제도 변화 등 HR 실무자가 인사이트를 얻을 수 있도록 작성
- 단락 구분은 \\n\\n 으로 처리
...
def run_newsletter():
    ...
    hr_ctx = ""
    for i, n in enumerate(hr_news):
        hr_ctx += f"[{i}] {n['title']} | {n['desc']}\n"
    deep_report = generate_deep_report(api_key, hr_ctx) if hr_news else None
